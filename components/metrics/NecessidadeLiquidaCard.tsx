...
export default React.memo(function NecessidadeLiquidaCard({ liquidNeed, totalExtraction, mosNContribution, soyNContribution, useDirectInput = false, animKey }: Props) {
  ...
  return (
    <MetricCard
      ...
      value={liquidNeed}
      unit="kg N/ha"
      formulaSummary={useDirectInput ? 'Valor informado diretamente' : 'Extração - MOS - Crédito Soja'}
      isDark={isDark}
      animKey={animKey}
    >
      <div className={`p-2.5 rounded-lg border font-mono text-[11px] leading-relaxed ${
        ...
        {useDirectInput ? (
          <>
            <div className={`font-bold ${isDark ? 'text-[#9CB386]' : 'text-[#5A5A40]'}`}>N_Liq = valor informado</div>
            <div className={`border-t my-1 pt-1 font-bold ${isDark ? 'border-[#2C3328] text-[#D4A373]' : 'border-[#F0EDE5] text-[#8D6E63]'}`}>
              N_Liq = {liquidNeed.toFixed(2)} kg N/ha
            </div>
            <p className={`mt-1 ${isDark ? 'text-[#9EA399]' : 'text-[#8C897E]'}`}>
              Necessidade líquida informada pelo usuário (MOS, Soja e extração já descontados).
            </p>
          </>
        ) : (
          <>
